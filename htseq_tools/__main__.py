...
def load_args():
    ...
    # Get Gene Lengths
    glen = sp.add_parser("gene_lengths", description="Extract gene exon lengths from GTF")
    glen.add_argument("--gtf_file", required=True, help="GTF file used for htseq count")
    glen.add_argument("--out_file", required=True, help="Output TSV file to write to")

    # Merge counts 
    merge = sp.add_parser("merge_counts", description="Merge PE and SE counts from HTseq")
    merge.add_argument("--htseq_counts", required=True, action='append',
        help="HTSeq count file. Use multiple times")
    merge.add_argument("--out_file", required=True, help="Output TSV file to write to")

    # FPKM
    fpkm = sp.add_parser("fpkm", description="Get FPKM and FPKM-UQ")
    fpkm.add_argument("--aggregate_length_file", required=True, help="Aggregate length TSV")
    fpkm.add_argument("--htseq_counts", required=True, help="HTSeq counts txt file")
    fpkm.add_argument("--output_prefix", required=True, help="The output prefix to use.")
