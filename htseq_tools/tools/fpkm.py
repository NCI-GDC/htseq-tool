...
def calculate_fpkm(all_read_count, pc_frag_count, all_gene_length, pc_genes,
    ...
    # Process and write
    with ofpkm_func(out_fpkm, 'wt') as ofpkm, ofpkm_uq_func(
                    out_fpkm_uq, 'wt') as ofpkm_uq:
        for gene in glist:
...
def main(args):
    ...
    gene_count, pcgenes, gene_length = load_protein_coding_ids(
        args.aggregate_length_file)

    logger.info("Getting fragment counts...")
    total_pc_frag_count, read_count_dic, gene_list = \
        get_protein_coding_read_count(pcgenes, args.htseq_counts)
